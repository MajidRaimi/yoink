...
yoink is a macOS-only CLI for switching between Claude Code accounts from your terminal. It stores each login as a named profile, swaps the credential in the macOS Keychain and the identity block in `~/.claude.json` on demand, and can also point Claude Code at external providers (Anthropic-compatible endpoints) with a managed env block. yoink ships as a single Bun-compiled binary. Package: [`yoink-cli`](https://www.npmjs.com/package/yoink-cli) on npm.

## Pages

| Page | What it covers |
| --- | --- |
| [Getting started](./getting-started.md) | Install on macOS, verify the version, add your first account. |
...
| [How it works](./how-it-works.md) | The Keychain, `~/.claude.json`, the switch sequence, and the files yoink touches. |
