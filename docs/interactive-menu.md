...
Switching from the menu behaves the same as [`yoink use`](./usage.md): it warns if Claude Code is running, and no-ops if the highlighted profile is already active. See [How it works](./how-it-works.md) for what a switch changes on disk and in the Keychain.

## Editing from the menu

Pressing `e` opens an editor that adapts to the highlighted profile's type:

- **Claude account:** the only editable field is the profile name, so `e` goes straight to a rename prompt. The Keychain login and `oauthAccount` identity are not touched.
